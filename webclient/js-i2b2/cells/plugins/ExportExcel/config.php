<?php
					...
					if( $_REQ["ACTION"] == 'Created' ){
						...
						if( $count == 0 ){
							$DB->query(
								...
								"VALUES ( -1, 'Created', '$javaClassName', '".$_ENC["PROJECTID"]."', '".$_ENC["PROJECTCODE"]."', '".f($params)."', '".f($_ENC["USERNAME"])."','".$ENV."' )"
							);
						} else {
							echo( "multiple?" );
						}
					} else {
						echo( "Invalid option selected, current job is ".$Job_settings["STATUS"] );
					}
				}
				$Job_settings = $DB->get_row( "SELECT I2B2_JOBS.*, ".getTime($DB,'LAST_TOUCHED')." AS TIMEOF FROM I2B2_JOBS WHERE PROJECTID=".$_ENC["PROJECTID"]." AND USERNAME='".f($_ENC["USERNAME"])."' AND JOB_TYPE='$javaClassName' ORDER BY JOB_ID DESC"  ,ARRAY_A );
			}
	...
	/**
	 * This function will try to figure out your DBMS and spew out a human readable time
	 * @returns SQL coded string representing the time
	 * @param $DB - this is a ezSQL instance, needed to see what DBMS you're using.
	 * @param $var - the variable desired.
	 */
	...
	/**
	 * This provides a method for checking passwords supplied.
	...
	function validatePassword( $user, $pass ){
		...
		if( function_exists( "ldap_connect" ) ){
			...
			$ldapconn = ldap_connect("ldap.urmc-sh.rochester.edu");
			if ($ldapconn) {
				...
				$ldapbind = ldap_bind($ldapconn, "URMC-SH\\".$user, $pass);
		...
		//CRI-531 : remove administrative i2b2 locks.
